pfUI:RegisterModule("uf_tukui", "vanilla:tbc", function ()
  ...
  if C.unitframes.player.layout == "tukui" then
    ...
    function pfUI.uf.player.UpdateConfig()
      ...
      pfUI.uf.player:SetHeight(pfUI.uf.player:GetHeight() + 2*default_border + (C.unitframes.player.caption_height) + pspacing)
      ...
      pfUI.uf.player.powerLeftText:SetPoint("LEFT",pfUI.uf.player.caption, "LEFT", default_border, 0)

      pfUI.uf.player.powerCenterText:SetParent(pfUI.uf.player.caption)
      pfUI.uf.player.powerCenterText:ClearAllPoints()
      pfUI.uf.player.powerCenterText:SetPoint("CENTER",pfUI.uf.player.caption, "CENTER", 0, 0)
  ...
  if C.unitframes.target.layout == "tukui" then
    ...
    function pfUI.uf.target.UpdateConfig()
      ...
      pfUI.uf.target:SetHeight(pfUI.uf.target:GetHeight() + 2*default_border + (C.unitframes.target.caption_height) + tspacing)
      ...
      pfUI.uf.target.caption:SetHeight(C.unitframes.target.caption_height)
      ...
      pfUI.uf.target.powerLeftText:SetPoint("LEFT",pfUI.uf.target.caption, "LEFT", default_border + C.unitframes.target.txtpowerleftoffx, C.unitframes.target.txtpowerleftoffy)

      pfUI.uf.target.powerCenterText:SetParent(pfUI.uf.target.caption)
      pfUI.uf.target.powerCenterText:ClearAllPoints()
      pfUI.uf.target.powerCenterText:SetPoint("CENTER",pfUI.uf.target.caption, "CENTER", 0, 0)
